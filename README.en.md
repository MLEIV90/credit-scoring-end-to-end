...
**▶️ [Try the live app](https://credit-scoring-end-to-end.streamlit.app/)** — submit an applicant and get the decision, score, and an explained reason.
...
## Reason codes with generative AI

An additional layer turns each decision into a natural-language reason for the
applicant, in line with the *adverse action notices* required by regulation
(ECOA in the US; right to explanation under GDPR/EBA in the EU). The design
principle is a **strict separation of responsibilities**:

- **The model decides.** The calibrated PD is mapped to a score and compared
  against the scorecard cutoff. Generative AI plays no part in the decision.
- **Logistic contributions quantify.** For each applicant, every feature's
  contribution to the log-odds (`coef × standardized value`) identifies the
  driving factors —exact, since it is the logistic model—. Factors are selected
  according to the decision (those pushing toward default on a rejection; those
  supporting it on an approval).
- **The LLM only writes.** It receives the decision and the pre-computed factors
  and produces the text; it never sees raw data or makes decisions. A
  **deterministic guardrail** filters the output to enforce the rules.

Model `llama-3.3-70b-versatile` via the Groq API, `temperature=0`. The web app
([Streamlit](https://credit-scoring-end-to-end.streamlit.app/)) lets you submit an
applicant through a form or evaluate a sample case.
